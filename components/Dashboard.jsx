...
function normalizeSelectedWeekendDates(raw) {
  if (Array.isArray(raw)) return normalizeSelectedDates(raw).map(toISODateString).filter(Boolean);
  return String(raw || "")
    .split(/[,;|\n]+/)
    .map((x) => x.trim())
    .filter(Boolean)
...
function applyWeekendBillingModeToDates(dates, weekendBillingMode = "include_all", selectedWeekendDates = []) {
  const normalizedDates = normalizeSelectedDates(dates);
  const mode = weekendBillingMode || "include_all";

  if (mode === "exclude_all") {
    return normalizedDates.filter((d) => !isWeekendDate(d));
  }

  if (mode === "selected_only") {
    const selectedSet = new Set(normalizeSelectedWeekendDates(selectedWeekendDates));
    return normalizedDates.filter((d) => {
      if (!isWeekendDate(d)) return true;
      return selectedSet.has(toISODateString(d));
    });
  }
...
function weekendBillingModeLabel(mode) {
  if (mode === "exclude_all") return "Exclude all weekends";
  if (mode === "selected_only") return "Include selected weekends only";
  return "Include all weekends";
}

function longTermPeriodLabel(period, customPeriod = "") {
  if (period === "3_months") return "3 months";
  if (period === "5_months") return "5 months";
  if (period === "1_year") return "1 year";
  if (period === "2_years") return "2 years";
  if (period === "5_years") return "5 years";
  if (period === "custom") return customPeriod || "Custom long-term period";
...
// phase3x-b-monthly-fixed-contract-pricing
function longTermPricingBasisLabel(value) {
  if (value === "monthly") return "Monthly Rate";
  if (value === "fixed") return "Fixed Contract Amount";
  return "Daily Rate";
}

function computeLongTermContractAmount(pricingBasis, months, monthlyRate, fixedAmount) {
  if (pricingBasis === "monthly") return clampMoney(clampMoney(monthlyRate) * clampMoney(months));
  if (pricingBasis === "fixed") return clampMoney(fixedAmount);
  return 0;
}

function getLongTermContractDisplayAmount(booking) {
  if (!booking || booking.hireType !== "long_term") return 0;
  if (booking.longTermPricingBasis === "monthly") return clampMoney(booking.longTermMonthlyRate || 0) * clampMoney(booking.longTermMonths || 0);
  if (booking.longTermPricingBasis === "fixed") return clampMoney(booking.longTermFixedAmount || 0);
  return 0;
}
...
function computeBookingCompleteTotalAmountFromTotals(totals, includeFuel, fuelAmount) {
  return clampMoney(totals?.totalNetClientAmount || 0) + (includeFuel ? clampMoney(fuelAmount || 0) : 0);
}

function computeItemSplit(rawItem, fallbackSelectedDates = [], cars = []) {
  const itemDates = applyWeekendBillingModeToDates(
    normalizeSelectedDates(
      Array.isArray(rawItem?.selectedDates) && rawItem.selectedDates.length
        ? rawItem.selectedDates
        : fallbackSelectedDates
    ),
    rawItem?.weekendBillingMode || "include_all",
    rawItem?.selectedWeekendDates || []
  );
  ...
  return {
    ...
    weekendBillingMode: rawItem.weekendBillingMode || "include_all",
    weekendBillingModeLabel: weekendBillingModeLabel(rawItem.weekendBillingMode || "include_all"),
    selectedWeekendDates: normalizeSelectedWeekendDates(rawItem.selectedWeekendDates || []),
...
export default function Dashboard() {
  ...
  const [longTermPricingBasis, setLongTermPricingBasis] = useState("daily");
  const [longTermMonthlyRate, setLongTermMonthlyRate] = useState("");
  const [longTermFixedAmount, setLongTermFixedAmount] = useState("");
  ...
  const [fuelInvoiceAmount, setFuelInvoiceAmount] = useState("");
  const [fuelInvoiceDescription, setFuelInvoiceDescription] = useState("Fuel billed to client");
  ...
  const formLongTermContractAmount =
    hireType === "long_term"
      ? computeLongTermContractAmount(
          longTermPricingBasis,
          formLongTermPeriodInfo.months,
          longTermMonthlyRate,
          longTermFixedAmount
        )
      : 0;

  const formItemsComputed = useMemo(() => bookingItems.map((item, idx) => {
    const isLongTermContractPriced =
      hireType === "long_term" &&
      (longTermPricingBasis === "monthly" || longTermPricingBasis === "fixed") &&
      formLongTermContractAmount > 0;

    const contractLineAmount = idx === 0 ? formLongTermContractAmount : 0;

    return computeItemSplit({
      ...item,
    ...
    }, isLongTermContractPriced ? [new Date()] : formEffectiveSelectedDates, cars);
  }), [bookingItems, cars, companyProfile, weekendBillingMode, selectedWeekendDates, hireType, longTermStartDate, longTermPeriod, customLongTermPeriod, selectedDates, longTermPricingBasis, longTermMonthlyRate, longTermFixedAmount, formLongTermContractAmount]);
  ...
  const loginWithEmail = async (e) => {
    ...
    try {
      ...
      setLoginError(err?.message || "Login failed. Check email/password and try again.");
    } finally {
  ...
  const resetBookingForm = () => {
    ...
    setBookingComments(""); setHireType("short_term"); setLongTermPeriod(""); setCustomLongTermPeriod(""); setLongTermStartDate(""); setLongTermPricingBasis("daily"); setLongTermMonthlyRate(""); setLongTermFixedAmount(""); setWeekendBillingMode("include_all"); setSelectedWeekendDates(""); setIncludeFuelOnInvoice(false); setFuelInvoiceAmount(""); setFuelInvoiceDescription("Fuel billed to client");
  };
  ...
  const saveBooking = async () => {
    ...
    const saveLongTermContractAmount =
      hireType === "long_term"
        ? computeLongTermContractAmount(
            longTermPricingBasis,
            saveLongTermPeriodInfo.months,
            longTermMonthlyRate,
            longTermFixedAmount
          )
        : 0;

    const computedItems = bookingItems.map((item, idx) => {
      const isLongTermContractPriced =
        hireType === "long_term" &&
        (longTermPricingBasis === "monthly" || longTermPricingBasis === "fixed") &&
        saveLongTermContractAmount > 0;

      const contractLineAmount = idx === 0 ? saveLongTermContractAmount : 0;
      ...
      return computeItemSplit({
        ...
        selectedWeekendDates: normalizeSelectedWeekendDates(selectedWeekendDates),
        clientVatMode: item.clientVatMode || getDefaultClientVatMode(companyProfile)
      }, isLongTermContractPriced ? [new Date()] : saveEffectiveSelectedDates, cars);
    });
    ...
    const bookingData = {
      ...
      comments: (bookingComments || "").trim(),
      hireType: hireType || "short_term",
      ...
      longTermPricingBasis: hireType === "long_term" ? (longTermPricingBasis || "daily") : "daily",
      longTermPricingBasisText: hireType === "long_term" ? longTermPricingBasisLabel(longTermPricingBasis || "daily") : "Daily Rate",
      longTermMonthlyRate: hireType === "long_term" && longTermPricingBasis === "monthly" ? clampMoney(longTermMonthlyRate) : 0,
      longTermFixedAmount: hireType === "long_term" && longTermPricingBasis === "fixed" ? clampMoney(longTermFixedAmount) : 0,
      longTermContractAmount: hireType === "long_term" ? saveLongTermContractAmount : 0,
      ...
      selectedWeekendDates: normalizeSelectedWeekendDates(selectedWeekendDates),
      includeFuelOnInvoice: Boolean(includeFuelOnInvoice),
      fuelInvoiceAmount: includeFuelOnInvoice ? clampMoney(fuelInvoiceAmount) : 0,
      fuelInvoiceDescription: includeFuelOnInvoice ? (fuelInvoiceDescription || "Fuel billed to client").trim() : "",
      ...
      chargeableDays: normSelected.length,
    ...
    if (editBookingId) {
      ...
      const updatedTotal = computeBookingCompleteTotalAmountFromTotals(pricingTotals, includeFuelOnInvoice, fuelInvoiceAmount);
  ...
  const editBooking = (b) => {
    ...
    setLongTermPricingBasis(b.longTermPricingBasis || "daily");
    setLongTermMonthlyRate(b.longTermMonthlyRate || "");
    setLongTermFixedAmount(b.longTermFixedAmount || "");
    ...
    setFuelInvoiceAmount(b.fuelInvoiceAmount || "");
    setFuelInvoiceDescription(b.fuelInvoiceDescription || "Fuel billed to client");
  ...
  const confirmBooking = async (b) => {
    ...
    const totals = getBookingTotals(b, cars);
    const amt = computeBookingTotalAmount({ ...b, pricingTotals: totals }, cars);
      ...
      <Card className="w-full max-w-md rounded-2xl shadow">
        <CardContent className="p-6 space-y-4">
          <div className="text-center space-y-1">
            <h1 className="text-2xl font-bold">MAALVILA Rental System</h1>
            <p className="text-sm text-gray-600">Sign in to load the dashboard.</p>
          </div>
          ...
          <div className="text-xs text-gray-500 text-center">
            Use the Firebase Auth email and password created for MAALVILA.
          </div>
        </CardContent>
      </Card>
    </div>
  ...
  const ClientDocumentPreview = ({ type, number, booking, notes = "" }) => {
    ...
    const fuelOnInvoiceAmount = getBookingFuelInvoiceAmount(booking);
    const finalDocumentTotal = computeClientDocumentFinalTotal(vat, booking);
    // phase3w-a-clean-document-options
    const documentSelectedWeekendDates = Array.isArray(booking?.selectedWeekendDates) ? booking.selectedWeekendDates.filter(Boolean) : [];
    const documentFuelAmount = getBookingFuelInvoiceAmount(booking);
    const hasLongTermInfo = booking?.hireType === "long_term" && (booking?.longTermPeriod || booking?.customLongTermPeriod || booking?.longTermPeriodText);
    const hasWeekendInfo = booking?.weekendBillingMode === "exclude_all" || booking?.weekendBillingMode === "selected_only" || documentSelectedWeekendDates.length > 0;
    const hasFuelInfo = Boolean(booking?.includeFuelOnInvoice) && documentFuelAmount > 0;
  ...
  const triggerDocumentPrint = (mode = "portrait") => {
    ...
    const existing = document.getElementById("maalvila-temporary-print-style");
    if (existing) existing.remove();

    if (mode === "landscape") {
      const style = document.createElement("style");
      style.id = "maalvila-temporary-print-style";
      style.innerHTML = "@media print { @page { size: A4 landscape; margin: 8mm; } .print-document { width: 100% !important; max-width: none !important; font-size: 10px !important; } .print-document table { width: 100% !important; table-layout: fixed !important; font-size: 9px !important; } .print-document th, .print-document td { padding: 3px !important; white-space: normal !important; word-break: break-word !important; overflow-wrap: anywhere !important; } .print-document .overflow-auto { overflow: visible !important; } }";
      document.head.appendChild(style);

      const cleanup = () => {
        const temp = document.getElementById("maalvila-temporary-print-style");
        if (temp) temp.remove();
      };
  ...
  // phase3v-f-bank-details-sort-swift
  const renderBankDetailsBox = () => (
    <div className="phase3t-bank-details border rounded-xl bg-blue-50 border-blue-200 p-3 text-sm space-y-2">
      ...
      <div className="grid md:grid-cols-2 gap-x-4 gap-y-1 text-blue-950">
        <div>Bank: <b>Stanbic Bank Ghana Ltd</b></div>
        ...
        <div>Sort Code: <b>190401</b></div>
        <div>SWIFT Code: <b>SBICGHAC</b></div>
  ...
  const graVatAuditRows = (() => {
    ...
    for (const booking of sourceBookings) {
      ...
      const items = getBookingItems(booking, cars);
      // phase3w-d3-gra-line-balance-fix
      ...
      const bookingFinalAmount = clampMoney((bookingVat.grandTotal || 0) + fuelAmount);
      const paymentStatus = booking.paymentStatus || computePaymentStatus(bookingFinalAmount, amountPaid);
      ...
      for (const item of items) {
        ...
        const lineGrandTotal = clampMoney(lineVat.grandTotal || 0);
        const lineShareRatio = bookingVat.grandTotal > 0 ? lineGrandTotal / bookingVat.grandTotal : 0;
        const lineFuelShare = booking.includeFuelOnInvoice ? clampMoney(fuelAmount * lineShareRatio) : 0;
        const lineFinalAmount = clampMoney(lineGrandTotal + lineFuelShare);
        const linePaidAllocation = bookingFinalAmount > 0 ? clampMoney(amountPaid * (lineFinalAmount / bookingFinalAmount)) : 0;
        const lineOutstandingBalance = Math.max(0, clampMoney(lineFinalAmount - linePaidAllocation));
        const lineTaxableBalance = lineFinalAmount > 0 ? clampMoney((clampMoney(lineVat.taxableAmount || 0) * lineOutstandingBalance) / lineFinalAmount) : 0;
        ...
        rows.push({
          ...
          fuelInvoiceAmount: lineFuelShare,
          lineFinalAmount,
          bookingFinalAmount,
          amountPaid: linePaidAllocation,
          taxableBalance: lineTaxableBalance,
          outstandingClientBalance: lineOutstandingBalance,
          balance: lineOutstandingBalance,
        });
      }
    }
  ...
  const graVatAuditSummary = (() => {
    return graVatAuditRows.reduce((acc, row) => {
      ...
      acc.finalAmount += row.lineFinalAmount || row.bookingFinalAmount || 0;
      acc.amountPaid += row.amountPaid;
      acc.taxableBalance += row.taxableBalance || 0;
      acc.balance += row.outstandingClientBalance || row.balance || 0;
  ...
  const exportGraVatAuditCSV = () => {
    ...
    const rows = [
      ...
      ...graVatAuditRows.map((row) => [
        ...
        row.lineFinalAmount || row.bookingFinalAmount,
        row.amountPaid,
        row.taxableBalance || 0,
        row.outstandingClientBalance || row.balance || 0,
      ]),
    ];
  ...
  const renderDocumentPrintControls = (label = "Document") => (
    <div className="phase3s-print-save-pdf no-print flex flex-wrap items-center justify-between gap-2 border-b pb-3 mb-3">
      <div>
        ...
        <div className="text-xs text-gray-500">Use Print or choose Save as PDF in the print window. Use Landscape PDF for wide quotes.</div>
      </div>
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => triggerDocumentPrint("portrait")}
  ...
  const SupplierVoucherPreview = ({ booking, sourceId, number, notes = "" }) => {
    ...
    return <div className="print-document border rounded-xl p-4 bg-white space-y-3"><style>{`@media print { body * { visibility: hidden !important; } .print-document, .print-document * { visibility: visible !important; } .print-document { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; box-shadow: none !important; border: none !important; } .no-print, .no-print * { display: none !important; visibility: hidden !important; } @page { margin: 12mm; } }`}</style>{renderDocumentPrintControls("Document Preview")}
      {renderBrandHeader()}
  ...
  return <div className="p-4 md:p-6 space-y-6 max-w-6xl mx-auto bg-slate-50 min-h-screen">
    ...
    {activeView === "dispatch" && <>
      <Card className="rounded-2xl shadow"><CardContent className="p-4 space-y-3">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div><h2 className="text-lg font-semibold">Dispatch Manifest - Next 14 Days</h2><div className="text-sm text-gray-600">Operational view for upcoming trips, drivers, vehicles, routes and supplier response status. Cancelled bookings are excluded. Balances are allocated by vehicle line to avoid overstating multi-car bookings. Overview Outstanding Balance may differ because Overview is a business receivables view, while this GRA/VAT report uses tax-report scope, selected period filters, cancelled-booking exclusion and line-level allocation.</div></div>
      ...
      <Card className="rounded-2xl shadow">
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 gap-3">
            ...
            <Card className="rounded-xl"><CardContent className="p-3"><div className="text-xs text-gray-500">Taxable Balance</div><div className="text-xs sm:text-sm font-semibold leading-snug break-words">{currencyGH(graVatAuditSummary.taxableBalance)}</div></CardContent></Card>
            <Card className="rounded-xl"><CardContent className="p-3"><div className="text-xs text-gray-500">Outstanding Client Balance</div><div className="text-xs sm:text-sm font-semibold leading-snug break-words">{currencyGH(graVatAuditSummary.balance)}</div></CardContent></Card>
          </div>
            ...
            <table className="w-full text-sm bg-white">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="p-2 text-right">Taxable Balance</th>
                  <th className="p-2 text-right">Outstanding Client Balance</th>
                </tr>
              </thead>
              <tbody>
                {graVatAuditRows.map((row) => (
                  <tr key={row.key} className="border-t">
                    ...
                    <td className="p-2 text-right">{currencyGH(row.taxableBalance || 0)}</td>
                    <td className="p-2 text-right">{currencyGH(row.outstandingClientBalance || row.balance || 0)}</td>
                  </tr>
                ))}
                {!graVatAuditRows.length && <tr><td colSpan={13} className="p-3 text-gray-500">No GRA/VAT audit rows found for the selected period.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
      ...
      <Card className="rounded-2xl shadow">
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <Card className="rounded-xl"><CardContent className="p-3"><div className="text-xs text-gray-500">Outstanding Client Balance</div><div className={`text-base md:text-lg font-bold leading-tight break-words ${fuelClaimsSummary.totalBalance > 0 ? "text-amber-700" : "text-emerald-700"}`}>{currencyGH(fuelClaimsSummary.totalBalance)}</div></CardContent></Card>
...
<Card className="rounded-2xl bg-slate-50 border-slate-200">
  <CardContent className="p-4 space-y-3">
    <div>
      <h3 className="font-semibold text-sm">Booking Options / Special Instructions</h3>
      <p className="text-xs text-gray-500">Use this section for long-term hire, weekend billing treatment, fuel billing and client-specific notes.</p>
    </div>

    <div className="grid md:grid-cols-3 gap-2">
      <div>
        <label className="text-xs text-gray-600">Hire Type</label>
        <select className="border rounded-lg p-2 text-sm h-10 w-full" value={hireType} onChange={(e) => setHireType(e.target.value)}>
          <option value="short_term">Short-term hire</option>
          <option value="long_term">Long-term hire</option>
        </select>
      </div>

      <div>
        <label className="text-xs text-gray-600">Long-Term Period</label>
        <select className="border rounded-lg p-2 text-sm h-10 w-full" value={longTermPeriod} onChange={(e) => setLongTermPeriod(e.target.value)} disabled={hireType !== "long_term"}>
      ...
      <div>
        <label className="text-xs text-gray-600">Custom Long-Term Period</label>
        <Input value={customLongTermPeriod} onChange={(e) => setCustomLongTermPeriod(e.target.value)} placeholder="e.g. 18 months, 4 years" disabled={hireType !== "long_term" || longTermPeriod !== "custom"} />
      </div>
    </div>
    ...
    <div className="grid md:grid-cols-4 gap-2">
      <div>
        <label className="text-xs text-gray-600">Long-Term Pricing Basis</label>
        <select className="border rounded-xl px-3 py-2 w-full" value={longTermPricingBasis} onChange={(e) => setLongTermPricingBasis(e.target.value)} disabled={hireType !== "long_term"}>
          <option value="daily">Daily Rate × Generated Days</option>
          <option value="monthly">Monthly Rate × Months</option>
          <option value="fixed">Fixed Contract Amount</option>
        </select>
      </div>
      <div>
        <label className="text-xs text-gray-600">Monthly Rate</label>
        <Input type="number" value={longTermMonthlyRate} onChange={(e) => setLongTermMonthlyRate(e.target.value)} placeholder="e.g. 20000" disabled={hireType !== "long_term" || longTermPricingBasis !== "monthly"} />
      </div>
      <div>
        <label className="text-xs text-gray-600">Fixed Contract Amount</label>
        <Input type="number" value={longTermFixedAmount} onChange={(e) => setLongTermFixedAmount(e.target.value)} placeholder="e.g. 60000" disabled={hireType !== "long_term" || longTermPricingBasis !== "fixed"} />
      </div>
      <div>
        <label className="text-xs text-gray-600">Contract Pricing Amount</label>
        <Input value={hireType === "long_term" && longTermPricingBasis !== "daily" ? currencyGH(formLongTermContractAmount) : "Uses daily rate"} readOnly disabled={hireType !== "long_term"} />
      </div>
    </div>

    {hireType === "long_term" ? <div className="text-xs text-blue-700 bg-blue-50 border border-blue-100 rounded-xl p-2">Long-term hire can calculate by daily rate, monthly rate, or fixed contract amount. Monthly/fixed pricing uses the contract amount for the client charge while keeping the generated period for dates, documents and audit trail.</div> : null}
